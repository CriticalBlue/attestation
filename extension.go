...
func populateMissingAuthorizationListFields(in *authorizationList, out *AuthorizationList) error {
	var rawValues []asn1.RawValue
	if _, err := asn1.Unmarshal(in.Raw, &rawValues); err != nil {
		return err
	}
	...
	for _, raw := range rawValues {
		...
		case TagUsageCountLimit:
			if out.UsageCountLimit == nil {
				limit, err := newOptionnalInt(raw)
				if err != nil {
					return err
				}
				out.UsageCountLimit = limit
			}
		}
	}
